    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                # 获取 softmax 概率图，形状: [C=3, D, H, W]
                softmax_probs = F.softmax(prediction, dim=0).cpu().numpy().astype(np.float32)

                # 保存为 NIfTI 文件：1 个文件包含两个通道
                nib.save(nib.Nifti1Image(softmax_probs, affine=np.eye(4)), ofile + "_prob_all.nii.gz")
                print(f"Saved liver + tumor probability heatmap to {ofile}_prob_all.nii.gz")
                # ===========================================
