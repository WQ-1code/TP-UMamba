...
def softmax_kernel(data, *, projection_matrix, is_query, normalize_data=False, eps=1e-4, device=None):
    ...
    # 添加梯度裁剪来防止数值不稳定
    data = torch.clamp(data, min=-50.0, max=50.0)
    ...
    # 使用torch.clamp确保不会有过大的指数值，避免梯度爆炸
    data_dash = torch.clamp(data_dash - diag_data, min=-50.0, max=50.0)
    
    # 增加eps值以避免数值不稳定
    data_dash = ratio * (torch.exp(data_dash) + eps)
...
def linear_attention(q, k, v):
    # 计算k的和并应用数值稳定技术
    k_cumsum = k.sum(dim=-2) + 1e-8  # 添加小常数防止除零
    
    # 计算注意力权重的逆
    D_inv = 1. / torch.clamp(torch.einsum('...nd,...d->...n', q, k_cumsum.type_as(q)), min=1e-8)
...
class ENLA(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        # 确保输入不包含NaN或Inf
        q = torch.nan_to_num(q, nan=0.0, posinf=1e5, neginf=-1e5)
        k = torch.nan_to_num(k, nan=0.0, posinf=1e5, neginf=-1e5)
        v = torch.nan_to_num(v, nan=0.0, posinf=1e5, neginf=-1e5)
...
class ENLTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads=4, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop=0.05, attn_drop=0.05, drop_path=0.05, act_layer=nn.GELU, norm_layer=nn.LayerNorm, kernel_size=1):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        # 添加小的缩放因子以稳定qk注意力
        self.qk_scale = 0.125 if qk_scale is None else qk_scale
        ...
        self.attn = ENLA(
            dim_heads=dim, 
            nb_features=min(dim, 256),  # 限制特征维度防止不稳定
            attn_drop=attn_drop,
            generalized_attention=True,  # 使用更稳定的通用注意力
            ortho_scaling=1  # 使用正交缩放
        )
    ...
    def forward(self, x):
        ...
        if self.use_checkpoint and torch.is_grad_enabled():
            def attention_block(x):
                ...
                x_assembly = self.conv_assembly(x)
                
                # 归一化嵌入，应用缩放因子
                x_embed_1 = F.normalize(x_embed_1, p=2, dim=1, eps=1e-5) * self.qk_scale
                x_embed_2 = F.normalize(x_embed_2, p=2, dim=1, eps=1e-5) * self.qk_scale
                ...
                return self.attn(x_embed_1, x_embed_2, x_assembly).squeeze(1)
            
            x = torch.utils.checkpoint.checkpoint(attention_block, x)
        else:
            # 常规前向传播
            x_embed_1 = self.conv_match1(x)
            x_embed_2 = self.conv_match2(x)
